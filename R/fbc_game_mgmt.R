...
deal_cards_to_grid <- function(deck = mmcards::shuffle_deck(), n) {
  ...
  # Convert the list of dealt cards into a list of lists (each representing a row)
  cards_matrix <- lapply(split(dealt_cards, rep(1:n, each=n)), function(row) as.list(row))
