#' Interpret Results of Informative Hypothesis Tests
#'
...
iht_interpreter <- function(iht_res, alpha=0.05) {
  ...
  if (pvalue_B > alpha) {
    explanation <- paste(explanation, "\nType B test is not significant (p =", round(pvalue_B, 3), "). This means we do not reject the null hypothesis that all restrictions hold in the population.")

    # Evaluate Type A test
    if (pvalue_A > alpha) {
      explanation <- paste(explanation, "\n\nType A test is also not significant (p =", round(pvalue_A, 3), "). This means we cannot conclude that at least one inequality constraint is strictly true.")
    } else {
      explanation <- paste(explanation, "\n\nType A test is significant (p =", round(pvalue_A, 3), "). This means at least one inequality constraint is strictly true.")
...
#' reduced (restricted) R-squared values to evaluate the imposed constraints.
#'
#' @param rm_res An object of class 'restriktor', typically the result of a call
#' to \code{\link[restriktor]{restriktor}}.
#'
#' @return A character string with a detailed interpretation of the analysis
